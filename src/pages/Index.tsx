...
const SectionHead = ({
  ...
  <div className={center ? "mx-auto max-w-2xl text-center" : "max-w-xl"}>
    <p
      className={cn(
        "flex items-center gap-3 text-[11px] font-semibold uppercase tracking-[0.2em]",
        center && "justify-center",
        inverted ? "text-background/45" : "text-muted-foreground",
      )}
    >
      <span className={cn("h-px w-7", inverted ? "bg-background/20" : "bg-foreground/15")} />
      {eyebrow}
    </p>
    <h2
      className={cn(
        "mt-4 text-balance font-display text-[1.9rem] font-semibold leading-[1.12] tracking-[-0.02em] sm:text-[2.5rem]",
    ...
    {sub && (
      <p className={cn("mt-4 text-[15px] leading-relaxed", inverted ? "text-background/50" : "text-muted-foreground")}>
        {sub}
      </p>
    )}
  </div>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
        ...
        {/* ===== COMMENT ÇA MARCHE (section sombre inversée) ===== */}
        <section id="comment" data-dark className="bg-foreground py-20 text-background lg:py-28">
          <Wrap>
            <SectionHead eyebrow="Comment ça marche" inverted>
              ...
              {steps.map(({ n, title, desc }, i) => {
                ...
                return (
                  <div key={n}>
                    ...
                    <div className="mt-5 flex items-center gap-3 border-t border-background/15 pt-5">
                      <span className="font-display text-sm font-semibold text-background/50">{n}</span>
                      <h3 className="font-display text-lg font-semibold">{title}</h3>
                    </div>
                    <p className="mt-2.5 text-sm font-light leading-relaxed text-background/50">{desc}</p>
                  </div>
                );
              })}
            </div>
            ...
            <div>
              ...
              <div className="mt-8 flex items-end gap-4">
                ...
                <div className="mb-1.5">
                  <p className="text-sm font-semibold">pour 1 USDT</p>
        ...
        <section data-dark className="bg-foreground py-20 text-background lg:py-24">
          <Wrap className="grid items-center gap-8 lg:grid-cols-[1.4fr_1fr]">
            <div>
              ...
              <p className="mt-5 max-w-md text-[15px] font-light leading-relaxed text-background/50">
            ...
            <div className="flex flex-wrap gap-3 lg:justify-end">
              <Link
                to="/connexion"
                className="inline-flex select-none items-center justify-center gap-2 rounded-full bg-background px-7 py-3 text-[15px] font-semibold text-foreground transition-all hover:opacity-90 active:scale-[0.98]"
              >
                Commencer <ArrowRight className="h-4 w-4" />
              </Link>
              <Link
                to="/connexion"
                className="inline-flex select-none items-center justify-center gap-2 rounded-full border border-background/25 px-7 py-3 text-[15px] font-semibold text-background transition-all hover:bg-background/10 active:scale-[0.98]"
              >
                Vendre des USDT
              </Link>
            </div>
