...
export function infoCard(rows, { title } = {}) {
  const head = title
    ? `<tr><td style="padding:2px 20px 10px;font-family:${FONT};font-size:11px;font-weight:700;
  ...
  const body = rows
    .map(
      ([label, value, mono], i) => `
    <tr>
      <td style="padding:${i === 0 ? "12" : "11"}px 20px;border-top:${i === 0 ? "0" : `1px solid ${C.border}`};font-family:${FONT};">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
          <td style="font-size:13px;color:${C.text};">${label}</td>
          <td align="right" style="font-size:13px;font-weight:600;color:${C.ink};${
